...
> 工具链:Hermes Kanban(任务源 + 派发)· Claude Code / OpenCode(实现)· git worktree · GitHub(代码真源 + PR 评审闸)· TeamCity(CI/构建/测试/打包)· itch.io(发布)。
...
| **TeamCity** | CI:push → 构建 + 跑测试(含不变量测试)+ 打包 | 要学 |
...
| **v1** | 加 TeamCity:push → 构建 + 跑**不变量测试** + 出可执行包 | 自动化链路 + **质量闸能不能自动拦住跑偏** | 同上 + handoff 证据(§4) |
...
- 同上,但 worker 完成时按 handoff 约定交证据(§4),TeamCity 在 PR 上跑不变量测试。
...
```json
{
  "changed_files": ["core/src/event_log.rs", "tests/invariants/replay.rs"],
  "verification": ["cargo test -p core invariants::"],
...
1. **CI 里不变量测试全绿**(TeamCity)——红线没被违反的物理证据。
...
**Hermes Kanban 当任务主导(第一阶段用骨、关编排魔法,你当 orchestrator)→ 单 SOTA worker 在 worktree 里 vibe coding → PR 上 TeamCity 跑不变量测试当物理闸 → 你靠测试+checklist+handoff+试跑验收,不读代码 → 飞轮三档点亮,第三阶段才放并行和小模型分层。**
