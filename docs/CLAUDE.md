...
## 0.5 运行现状(CURRENT ARCHITECTURE · 权威 · 覆盖下文 §4/§6 的冻结表述)

> 下文 §4「技术栈」与 §6「目录约定」描述的是**冻结的远期目标**(TECH-SPEC),**不是当前运行形态**。开工前以本节为准:
>
> - **当前形态 = localhost web 应用。** Rust `core` 编译成 **WASM** 在浏览器运行,store 是**内存版**(`core/src/memory_store.rs`);可选的原生 `workbench-server`(`cargo run -p workbench-core --bin workbench-server`)在 **localhost:5198** 提供真实 AI 提议。前端 = Vite + **React**(已定,不再是 React/Svelte 二选一)+ React Flow + Three.js,`cd ui && npm run dev`(默认端口 5173)。
> - **尚未落地(冻结/远期,别照着建运行时):** Tauri 外壳、SQLite 持久层、`butler`/itch 发布。仓库里**没有** `src-tauri/`,Cargo 里**没有** tauri/sqlite 依赖。
> - **不受影响:** §1 的 INV-1..8 红线 + 单一契约边界(`WorkbenchCore` 唯一导出)**是准确的、且已编码成测试**(`core/src/invariant_tests.rs`),无论外壳是 web 还是将来的 Tauri 都成立。
> - **已知的、有意保留的 INV-4 违例:** `core/src/cli_bridge.rs`(`std::process` 调 opencode)与 `core/src/cli_server.rs`(`std::net` 监听 :5198)是 `#[cfg(native)]`-gated 的**原型期遗留**,已建卡(`workbench-harden` 板 `t_281dddf1`)排期迁出 core——**不要顺手"清理"或删除它们**。
> - **当前开发组织 / 看板状态 / 如何派活 / 下一步,见根目录 `AGENTS.md`**(每个 agent 开工先读它)。
...
- 这些测试进 CI(**GitHub Actions** · `.github/workflows/ci.yml`),push / PR 即跑。
...
实际当前布局(冻结目标里的 `/src-tauri`、独立 `/tests` 目录**尚不存在**):
```
/core              # Rust crate:确定性核心 + 事件溯源 + 单一契约(无 LLM/无渲染)
                   #   core/src/invariant_tests.rs = §3 的护栏测试(不是独立 /tests 目录)
                   #   core/src/{cli_bridge,cli_server}.rs = native-gated AI bridge(待迁出,见 0.5)
                   #   core/src/wasm.rs = WASM IPC 边界;core/src/bin/server.rs = workbench-server
/ui                # 前端(TS + React + Vite),所有渲染都在这;ui/src/core-pkg/ = 编译产物 WASM(已提交)
/scripts           # run-worker.sh 等编排脚本
/.github/workflows # ci.yml(GitHub Actions:cargo test + ui build)
/docs              # CLAUDE.md SPEC-v1.md TECH-SPEC.md WORKFLOW.md DESIGN.md MVP-ACCEPTANCE.md
AGENTS.md          # 根目录:agent 开工先读(运行/构建/派活/当前状态)
```
（远期若引入 Tauri,会新增 `/src-tauri`;届时更新本表。）
